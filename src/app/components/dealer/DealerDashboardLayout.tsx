...
import {
  LayoutDashboard, Package, MapPin, MessageSquare, BarChart2,
  ShoppingCart, Settings, Building2, Heart,
...
const NAV = [
  { label: "Dashboard",     href: "/dealer",           icon: LayoutDashboard, exact: true },
  { label: "Inventory",     href: "/dealer/inventory", icon: Package },
  { label: "Service Areas", href: "/dealer/areas",     icon: MapPin },
  { label: "Orders",        href: "/dealer/orders",    icon: ShoppingCart },
  { label: "Messages",      href: "/dealer/messages",  icon: MessageSquare },
  { label: "Wishlist",      href: "/dealer/wishlist",  icon: Heart },
];

const BOTTOM_NAV = [
  { label: "Analytics", href: "/dealer/analytics", icon: BarChart2 },
  { label: "Profile",   href: "/dealer/profile",   icon: Building2 },
  { label: "Settings",  href: "/dealer/settings",  icon: Settings },
];

const NOTIFICATIONS = [
  { icon: MessageSquare, color: "#f59e0b", text: "New enquiry from Morphogenesis for TMT Bars — ₹2.4L potential", time: "30m ago",  unread: true  },
  { icon: Package,       color: "#10b981", text: "Your listing 'UltraTech OPC 53' has 42 new views this week",  time: "2h ago",  unread: true  },
  { icon: MapPin,        color: "#8b5cf6", text: "Service area 400097 (Borivali) is currently paused",           time: "1d ago",  unread: false },
  { icon: ShoppingCart,  color: "#0284c7", text: "Order #DL-2048 from Studio Sangam confirmed",                  time: "2d ago",  unread: false },
];
...
export function DealerDashboardLayout() {
  ...
  const [showTour, setShowTour]     = useState(() => !isTourDone("dealer"));
  const location = useLocation();
  const notifRef = useRef<HTMLDivElement>(null);

  function startTour() { resetTour("dealer"); setShowTour(true); setMobileOpen(false); }
  function closeTour() { markTourDone("dealer"); setShowTour(false); }

  const STEPS: TourStep[] = [
    {
      icon: <span style={{ fontSize: "2rem" }}>🏪</span>,
      title: "Welcome to your dealer dashboard!",
      body: "Your command centre for managing inventory listed on Material Library and the pin codes you service. Let's take a quick look.",
      anchor: "center",
      targetId: "tour-dealer-stats",
    },
    {
      icon: <Package className="w-7 h-7" style={{ color: ACCENT }} />,
      title: "See your top-performing inventory",
      body: "Track which products are getting the most views and enquiries from designers on the platform. Update listings to stay competitive.",
      anchor: "bottom-right",
      hint: "↑ Inventory summary is above",
      targetId: "tour-dealer-inventory",
    },
    {
      icon: <MapPin className="w-7 h-7" style={{ color: ACCENT }} />,
      title: "Manage your serviceable areas",
      body: "Active pin codes determine where you appear in location-based product searches. Keep them accurate to get relevant enquiries.",
      anchor: "bottom-right",
      hint: "↑ Service areas are above",
      targetId: "tour-dealer-areas",
    },
    {
      icon: <LayoutDashboard className="w-7 h-7" style={{ color: ACCENT }} />,
      title: "Navigate your dealer tools",
      body: "The sidebar gives you access to Inventory, Service Areas, Orders, Messages, Analytics, and your Wishlist.",
      anchor: "bottom-left",
      hint: "← Sidebar is to the left",
      targetId: "tour-dealer-sidebar",
    },
  ];
  ...
  const initials  = authUser?.initials  ?? "BM";
  const firstName = authUser?.name?.split(" ")[0] ?? "BuildMart";
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full">
      ...
      <div className="px-2 flex-shrink-0 space-y-0.5" style={{ borderTop: "1px solid rgba(0,0,0,0.06)", paddingTop: 8 }}>
        ...
        <Link
          to={authUser?.profilePath ?? "/store/buildmart-india"}
          className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all"
          ...
          >
            ...
            <div className="min-w-0">
              <div
                ...
                className="truncate"
              >
                {authUser?.name ?? "BuildMart India"}
              </div>
              <div style={{ fontSize: "0.68rem", color: "var(--text-muted)" }}>
                {authUser?.type ?? "Dealer"}
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col h-screen" style={{ background: "#fffbeb" }}>
      <Navbar />

      <div className="flex flex-1 overflow-hidden">
        <aside
          id="tour-dealer-sidebar"
                  ...
                  {notifOpen && (
                    ...
                    >
                      ...
                      >
                        ...
                        {unreadCount > 0 && (
                          <span className="px-2 py-0.5 rounded-full text-[10px] font-bold text-white" style={{ background: "#FF6A3D" }}>
                            {unreadCount} new
                          </span>
                        )}
                      </div>
