...
const SPECIALIZATIONS = [
  "Interior Design", "Architecture", "Landscape Architecture",
  "Visualization / Rendering", "Urban Design", "Product Design",
];
const EXPERIENCE_RANGES = ["0–1 years", "2–4 years", "5–9 years", "10+ years"];
const TOOLS = [
  "AutoCAD", "Revit", "SketchUp", "3ds Max", "V-Ray", "Corona",
  "Rhino", "Grasshopper", "Lumion", "Photoshop", "InDesign",
  "Illustrator", "Enscape", "ArchiCAD", "Hand Sketching",
];
...
export function DesignerOnboarding({ onComplete }: DesignerOnboardingProps) {
  ...
  function handleComplete() {
    localStorage.setItem("ml_designer_onboarded", "true");
    onComplete();
  }
  ...
  const canProceedStep1 = specialization && experience;
        ...
        <div className="px-6 pt-6 pb-5" style={{ borderBottom: "1px solid rgba(0,0,0,0.06)" }}>
          ...
          <p style={{ fontSize: "0.82rem", color: "var(--text-muted)", marginTop: 4 }}>
            {step === 1
              ...
              : step === 2
              ? "Select your design tools — we'll surface relevant jobs and courses."
              : "Add your portfolio so clients and studios can discover your work."}
          </p>
        </div>
          ...
          <AnimatePresence mode="wait">
            {step === 1 && (
              ...
              >
                  ...
                  <div className="mt-1.5 grid grid-cols-2 gap-2">
                    {SPECIALIZATIONS.map((s) => (
                      <button
                        ...
                        style={{
                          background: specialization === s ? `rgba(139,92,246,0.12)` : "rgba(0,0,0,0.04)",
                          color: specialization === s ? ACCENT : "var(--text-secondary)",
                          border: specialization === s ? `1px solid rgba(139,92,246,0.3)` : "1px solid rgba(0,0,0,0.06)",
                        }}
                      >
                        {specialization === s && "✓ "}
                        {s}
                  ...
                  <div className="mt-1.5 grid grid-cols-4 gap-2">
                    {EXPERIENCE_RANGES.map((e) => (
                      <button
                        ...
                        style={{
                          background: experience === e ? ACCENT : "rgba(0,0,0,0.04)",
                          color: experience === e ? "white" : "var(--text-secondary)",
                          border: experience === e ? `1px solid ${ACCENT}` : "1px solid rgba(0,0,0,0.08)",
                        }}
                      >
                        {e}
                      </button>
                ...
                <div>
                  <label className="text-[12px] font-semibold" style={{ color: "var(--text-secondary)" }}>
                    Based in (City) <span style={{ color: "var(--text-muted)", fontWeight: 400 }}>(optional)</span>
                  </label>
                  <input
                    type="text"
                    value={city}
                  ...
                  {TOOLS.map((t) => {
                    ...
                    return (
                      <button
                        ...
                        style={{
                          background: selected ? "rgba(139,92,246,0.12)" : "rgba(0,0,0,0.04)",
                          color: selected ? ACCENT : "var(--text-secondary)",
                          border: selected ? "1px solid rgba(139,92,246,0.3)" : "1px solid rgba(0,0,0,0.06)",
                        }}
                      >
                        {selected && "✓ "}
                        {t}
              ...
              >
                ...
                >
                  ...
                  <p style={{ fontSize: "0.8rem", color: "var(--text-secondary)", lineHeight: 1.5 }}>
                    Clients and studios browse designer portfolios on Material Library.{" "}
                    <strong>Add your portfolio to start receiving enquiries.</strong>
                  </p>
                </div>
